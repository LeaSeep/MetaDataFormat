Sub CompareAndMergeSheets()
    ...
    For Each compareSheet In ThisWorkbook.Sheets
        If compareSheet.Index <> baseSheet.Index Then
            ...
            baseLastColumn = baseSheet.Cells(2, baseSheet.Columns.Count).End(xlToLeft).Column
            compareLastColumn = compareSheet.Cells(2, compareSheet.Columns.Count).End(xlToLeft).Column
            
            ' Loop through each cell in the 6th row of the compare sheet
            For Each baseColumn In baseSheet.Range(baseSheet.Cells(2, 2), baseSheet.Cells(6, baseLastColumn))
                For Each compareColumn In compareSheet.Range(compareSheet.Cells(2, 2), compareSheet.Cells(2, compareLastColumn))
                    ' Check if the column names match
                    If baseColumn.Value = compareColumn.Value Then
                        ' Find added items in the compare sheet
                    

                        base_col = baseSheet.Rows(2).Find(What:=baseColumn.Value, LookIn:=xlValues, LookAt:=xlWhole).Column
                        cmp_col = compareSheet.Rows(2).Find(What:=compareColumn.Value, LookIn:=xlValues, LookAt:=xlWhole).Column
                        ...
                        If WorksheetFunction.CountA(Range(base_col_Letter & ":" & base_col_Letter)) = 0 Then
                         'empty
                            Exit For
                         Else
                            NextFree_base = baseSheet.Rows(2).Columns(base_col).End(xlDown).Offset(1, 0).Row
                        End If
                        
                         If WorksheetFunction.CountA(Range(cmp_col_Letter & ":" & cmp_col_Letter)) = 0 Then
                         'empty
                            Exit For
                         Else
                            NextFree_cmp = compareSheet.Rows(2).Columns(cmp_col).End(xlDown).Offset(1, 0).Row
